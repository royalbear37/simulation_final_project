...
def total_idle_time(allocation, sim_time, dispatch_rule='FIFO', ga_priority=None):
    e_idle = simulate_idle_time(ETCH_machines, allocation[0], sim_time, dispatch_rule=dispatch_rule, ga_priority=ga_priority)
    p_idle = simulate_idle_time(PHOTO_machines, allocation[1], sim_time, dispatch_rule=dispatch_rule, ga_priority=ga_priority)
    t_idle = simulate_idle_time(TF_machines, allocation[2], sim_time, dispatch_rule=dispatch_rule, ga_priority=ga_priority)
    print("="*40)
    allocation = [int(x) for x in allocation]
    print(f"Allocation: {allocation}")
    print(f"Dispatch rule: {dispatch_rule}")
    if ga_priority:
        print(f"GA priority: {ga_priority}")
